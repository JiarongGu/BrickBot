...
public sealed class DetectionTrainerService : IDetectionTrainerService
{
    ...
    private TrainingResult TrainElement(string profileId, TrainingSample[] samples, DetectionDefinition? seed)
    {
        ...
        try
        {
            ...
            // Persist as a new template row + PNG.
            var pngBytes = medianTemplate.ImEncode(".png");
            var templateName = (seed?.Name ?? "trained-element").Trim();
            if (string.IsNullOrEmpty(templateName)) templateName = "trained-element";
            var savedTemplate = _templateFiles.SavePngAsync(profileId, "", templateName, "Auto-generated by trainer", pngBytes)
            ...
            tpl.TemplateName = savedTemplate.Id;  // store id, not user name
            ...
            return new TrainingResult
            {
                Suggested = suggested,
                Diagnostics = diagnostics,
                Summary = $"template {savedTemplate.Name} ({medianTemplate.Width}×{medianTemplate.Height}) · " +
